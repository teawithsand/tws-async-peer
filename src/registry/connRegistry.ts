 ...
 */
export type ConnRegistryAdapterHandle<T, S, C, I> = Readonly<{
	id: string
	conn: T
	initData: I
 ...
 * 
 * It accepts adapter responsible for actual connection handling logic. Adapter is, of course allowed
 * to have it's own internal state, but state here can be accessed outside.
 * 
 * State here is not type provided by this library. 
 * Instead it should be your state for handling whatever domain logic you would like to handle.
 * 
 * ## Removing connections
 * This registry does not remove connections automatically once they ended. 
 * It's user responsibility to call `removeConn`. This way user can read data of any ended connection.
 *
 * ## Generic types
 * T - type of connection that comes from. This is like raw connection with no domain logic.
 * S - State of each connection managed by adapter. Can be read outside of adapter, so things stored here
 *     can be displayed on UI or sth.
 * C - Config of each connection. Used to send data to adapter. can be read outside of adapter.
 * I - Initial data of connection that comes. This could be merged with T, but in for some reason in the past
 *     I didn't do so. It's also available outside adapter and unlike config and state can't be mutated 
 *     during connections lifecycle.
 * 
 * ## If you are using react...
 * Check out `useStickyEventBus` from `tws-lts-react` and subscribe to `innerStateBus`
